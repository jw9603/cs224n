...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    outsideSoftmaxVec= softmax(outsideVectors@centerWordVec)
    loss = -np.log(outsideSoftmaxVec[outsideWordIdx])
    gradCenterVec = -outsideVectors[outsideWordIdx] + outsideVectors.T @ outsideSoftmaxVec
    gradOutsideVecs = np.reshape(outsideSoftmaxVec,(-1,1)) @ np.reshape(centerWordVec,(1,-1))
    gradOutsideVecs[outsideWordIdx] = (outsideSoftmaxVec[outsideWordIdx]-1) * centerWordVec
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    sigmoid_neg_score_ks = np.reshape(sigmoid(-scores[1:]), (-1, 1)) # (K, 1)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        loss_o, gradCenterVecs_o, gradOutsideVectors_o = word2vecLossAndGradient(
            centerWordVec, word2Ind[outsideWord], outsideVectors, dataset)
        loss += loss_o
        gradCenterVecs[centerWordInd] += gradCenterVecs_o
        gradOutsideVectors += gradOutsideVectors_o
    ### END YOUR CODE
